...
type NodeT = { id: string; kind: "dir" | "file"; label: string; file?: string };
type EdgeT = { from: string; to: string; type: "child" | "imports" };

const isCodeFile = (p: string) => /\.(js|jsx|ts|tsx|py|tsc|tsx)$/i.test(p);

// Build a Python module name from a repo-relative path: "dir/file.py" -> "dir.file"
function pyModuleName(repoRoot: string, absPath: string) {
  const rel = path.relative(repoRoot, absPath).replace(/\\/g, "/");
  if (!rel.toLowerCase().endsWith(".py")) return null;
  return rel.slice(0, -3).replace(/\//g, ".");
}

export async function analyzeRepo(repoRoot: string) {
  // 1) find all files (case-insensitive)
  const rels = glob.sync("**/*", {
    cwd: repoRoot,
    nocase: true,
  ...
  const dirId = (abs: string) => `dir:${abs}`;
  const fileId = (abs: string) => `file:${abs}`;
  ...
  const pyModuleToFile = new Map<string, string>();
  for (const f of files) {
    if (f.toLowerCase().endsWith(".py")) {
      const mod = pyModuleName(repoRoot, f);
      if (mod) pyModuleToFile.set(mod, f);
    }
  }
  ...
  for (const f of files) {
    ...
    for (const m of code.matchAll(FROM_RE)) {
      ...
      for (const name of names) {
        ...
        if (exact) edges.push({ from: fileId(f), to: fileId(exact), type: "imports" });
        else {
          const baseFile = pyModuleToFile.get(base);
          if (baseFile) edges.push({ from: fileId(f), to: fileId(baseFile), type: "imports" });
        }
      }
    }
    // import a, b.c
    for (const m of code.matchAll(IMPORT_RE)) {
      ...
      for (const mod of list) {
        ...
        if (target) edges.push({ from: fileId(f), to: fileId(target), type: "imports" });
      }
    }
  }
